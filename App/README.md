...
- Reopen source selection later with the Scan Sources button
...
- Saved source choices: `Codebase/settings.json`
...
- Source discovery is dynamic for the current Windows user and resolves local session folders from `%USERPROFILE%`.
...
- Scan Sources controls which local tools/folders are scanned; All Models, Claude, Codex, and Grok are filters for already discovered rows.
